...
# Load SLACK_REVIEW_WEBHOOK_URL from .env if not already set
if [[ -z "${SLACK_REVIEW_WEBHOOK_URL:-}" && -f .env ]]; then
  val=$(grep -E '^SLACK_REVIEW_WEBHOOK_URL=' .env 2>/dev/null | head -1 | cut -d= -f2- | sed "s/^['\"]//;s/['\"]$//" | tr -d '\r') || true
  if [[ -n "$val" ]]; then
    export SLACK_REVIEW_WEBHOOK_URL="$val"
  fi
fi
